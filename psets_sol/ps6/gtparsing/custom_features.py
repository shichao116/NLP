...
class W2V_ClusterFeats(ContextFeats):
    def create_arc_features(self,instance,h,m,add=False):
        ff = super(W2V_ClusterFeats,self).create_arc_features(instance,h,m,add)
        k = len(ff)
        hwid = instance.words[h]
        mwid = instance.words[m]
        if hwid != -1 and mwid != -1:
            hw = self.word_dict[hwid]
            mw = self.word_dict[mwid]
            if (hw in self.words2index) and (mw in self.words2index):
                f = self.getF((k,instance.pos[h],self.cluster[self.words2index[hw]],\
                                 instance.pos[m],self.cluster[self.words2index[mw]]), add)
                ff.append(f)
                k = len(ff)
            elif hw in self.words2index:
                f = self.getF((k,instance.pos[h],self.cluster[self.words2index[hw]]),add)
                k = len(ff)
            elif mw in self.words2index:
                f = self.getF((k,instance.pos[m],self.cluster[self.words2index[mw]]),add)
                k = len(ff)
        return ff
